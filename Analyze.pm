...
sub lsqfit {
  ...
  my $refatom=$self->_getRefAtom($self->{refmol},$resnumonly,$renumber);
...
sub _getRefAtom {
  ...
  if (!defined $self->{_refatom}) {
    ...
    foreach my $c ( @{$mol->activeChains()} ) {
      foreach my $a ( @{$c->{atom}} ) {
	$self->{_refatom}->{&_getKey($a,undef,undef,$resnumonly,$renumber)}=$a;
	$self->{_refatom}->{&_getKey($a,undef,0,$resnumonly,$renumber)}=$a
	    if (!$self->{_multiref});
      }
    }
  }
...
#      printf STDERR ("%s/%s\n",$cref,$calign);
	...
	printf STDERR "residue name does not match in aligned PDB\n";
	exit 1;
      }
