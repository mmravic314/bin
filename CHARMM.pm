...
sub setupFromMolecule {
  ...
  if ($terminal eq "blocked" && !$readonly) {
    if ($self->{par}->{param} eq "19" || $self->{par}->{param} eq "eef1") {
  ...
  for (my $if=0; $if<=$#{$slist}; $if++) {
	...
	for (my $ifn=0; $ifn<=$#narr; $ifn++) {
      ...
      } elsif ($self->{par}->{param} eq "19" || $self->{par}->{param} eq "eef1") {
	$first="firs amnp";
	$last="last cbxp";
      } elsif ($self->{par}->{param} =~ /22/ || $self->{par}->{param} =~ /27/) {
	$first="firs ace";
	$last="last ct3";
      }	
    }
    ...
    $segmol->fixCOO($self->{par}->{blocked} && (!defined $self->{par}->{cter} || lc $self->{par}->{cter} ne "cter")) if (!$hetero);
    $segmol->writePDB($fname,translate=>getConvType($self->{par}->{param}),ssbond=>0);


    if (!$readonly) {
      if ($f->{name} eq "TIP3" || $f->{name}=~/^W/) {
	my $trec={};
	$trec->{fname}=$fname;
...
#  $self->_sendCommand("ic fill ");
    ...
    if (($self->{par}->{param} eq "19" || $self->{par}->{param} =~ /22/ ||
         $self->{par}->{param} eq "27" || 
	 $self->{par}->{param} eq "eef1" || $self->{par}->{param} eq "lpdb")) {
...
sub OldreadFromPDB {
  ...
  $mol->fixCOO($self->{par}->{blocked} && (!defined $self->{par}->{cter} || lc $self->{par}->{cter} ne "cter"));
...
sub setupNonBonded {
  ...
  if ($self->{par}->{cut}) {
    ...
    $self->_sendCommand("update $self->{par}->{nbmode} $self->{par}->{dielec} eps $self->{par}->{epsilon} cutnb $self->{par}->{cutnb} ctofnb $self->{par}->{cutoff} ctonnb $self->{par}->{cuton} $trunc $vtrunc $lrc bygr");
  } else {
    my $trunc="switch";
    my $vtrunc="vswitch";
    ...
    $self->_sendCommand("update $self->{par}->{nbmode} $self->{par}->{dielec} eps $self->{par}->{epsilon} cutnb 9999.0 ctofnb 9000.0 ctonnb 9000.0 $trunc $vtrunc $lrc bygr");
  }
}
...
sub periodicBoundaries {
  ...
  if (defined $self->{par}->{imbyseg}) {
    ...
    $strbyseg=".not. resname TIP3 .and. .not. resname CLA .and. .not. resname SOD";
  }
  ...
  if (defined $self->{par}->{imbyres}) {
    ...
    $strbyres="resname TIP3 .or. resname CLA .or. resname SOD";
  }
    
  $self->_sendCommand("image byseg xcen 0.0 ycen 0.0 zcen 0.0 select $strbyseg end") if (defined $strbyseg && $strbyseg ne "");
  $self->_sendCommand("image byres xcen 0.0 ycen 0.0 zcen 0.0 select $strbyres end") if (defined $strbyres && $strbyres ne "");

  my $cutim=$self->{par}->{cutnb}+3.0;
...
sub ewald {
  ...
  my $kappa=(defined $self->{par}->{pmekappa})?$self->{par}->{pmekappa}:5.0/$self->{par}->{cutoff};
  ...
  $self->_sendCommand("update ewald kappa $kappa kmax $kmax pmewald order 4 fftx $npmex ffty $npmey fftz $npmez $qcor");
...
sub runDynamics {
  ...
  my $cmd;
  if (!$self->{par}->{lang} && !$self->{par}->{sgld}) {
    if ($self->{par}->{sgmd}) {
...
#	  $cmd="dyna leap nose rstn qref $self->{par}->{dynnoseq} tref $self->{par}->{dyntemp} ncyc $self->{par}->{dynnosen} ";
    ...
    if ($self->{par}->{sgld}) {
      $cmd="dyna leap lang sgld tsgavg $self->{par}->{sgldtsvg} tsgavp $self->{par}->{sgldtsvp} -\n";
...
sub getBiasVal {
  ...
  if ($par{type} eq "rg") {
    ...
    my $cx=substr($cstr,0,10);
    my $cy=substr($cstr,10,10);
    my $cz=substr($cstr,20,10);
...
#    $cmd=sprintf("define DIHESEL4 %s\n", $self->_makeSelection($par{sel4}));
#    $self->_sendCommand($cmd);
    my $cmd=sprintf("q sele DIHESEL1 end sele DIHESEL2 end -\n");
    $cmd.=sprintf("sele DIHESEL3 end sele DIHESEL4 end\n");
    $self->_sendCommand($cmd);
...
sub quasiHarmonicAnalysis {
  ...
  my $cmd=sprintf("vibran nmode %d\n",$totmodes);
  $cmd.=sprintf("quasi nfreq %d nadd %d nunit 1 firstu 22 sele all end temp %f\n",
		$nmodes,$totmodes-$nmodes,$temperature);
...
sub poissonBoltzmann {
  ...
  $self->_sendCommand(sprintf("SOLVE epsp $self->{par}->{epsp} epsw $self->{par}->{epsw} nclx %d ncly %d nclz %d dcel %1.4f xbce %1.4f ybce %1.4f zbce %1.4f maxi 10000 intbp %s %s",$nxcel,$nycel,$nzcel,$self->{par}->{dcel},$cstat->{xcenter},$cstat->{ycenter},$cstat->{zcenter},$surface,$self->{par}->{pbextra}));
...
sub pbgrid {
  ...
  $self->_sendCommand(sprintf("SOLVE epsp $self->{par}->{epsp} epsw $self->{par}->{epsw} nclx %d ncly %d nclz %d dcel %1.4f xbce %1.4f ybce %1.4f zbce %1.4f maxi 10000 intbp %s",$nxcel,$nycel,$nzcel,$self->{par}->{dcel},0.0,0.0,0.0,$surface));
  $self->_sendCommand("write card $name unit 20 xfirst -$tmaxx xlast +$tmaxx yfirst -$tmaxy ylast +$tmaxy zfirst -$tmaxz zlast +$tmaxz");

  $self->_sendCommand("RESET");
  $self->_sendCommand("END");

  return ($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel);
}
...
sub epsgrid {
  ...
  $self->_sendCommand(sprintf("SOLVE epsp $self->{par}->{epsp} epsw $self->{par}->{epsw} nclx %d ncly %d nclz %d dcel %1.4f xbce %1.4f ybce %1.4f zbce %1.4f maxi 1 intbp %s",$nxcel,$nycel,$nzcel,$self->{par}->{dcel},0.0,0.0,0.0,$surface));
...
## function: analyzeTrajectoryOrient([options])

sub analyzeTrajectoryOrient {
  my $self=shift;
  my $fname=shift;
  my %par=@_;
  ...
  my $cmd=sprintf("merge firstu 22 outputu 23 select all end orient $mass select $par{fitsel} end");
  $self->_sendCommand($cmd);
}
