...
# python ~/bin/extractTrimerTM.py ~/jan2016TMpdb/non_redund.txt ~/jan2016TMpdb/TM_redundant/ ~/jan2016TMpdb/3_helix/ ~/jan2016TMpdb/extendedDefinitions.txt segLabeled_nonRedun-TM/ 18
...
def extract_windows( inPDB, win=14):
	...
	A = np.arange(  0, h_len[0] - win-1 )
	B = np.arange(  h_len[0] +1, h_len[1] - win-1 )
	C = np.arange(  h_len[1] +1, h_len[2] -win -1 )
...
#	print C
	...
	for h1 in A:
		for h2 in B:
			for h3 in C:
				arng 	= np.arange( h1, h1 + win ) 
				brng 	= np.arange( h2, h2 + win )
				crng 	= np.arange( h3, h3 + win )
	...
	for i in sorted( minVals, reverse = True ):
		...
		A2, B2, C2 = [], [], []
		for a in A:
			if a <= h1 - overlap or a >= h1 + overlap: A2.append(a)
		for b in B:
			if b <= h2 - overlap or b >= h2 + overlap: B2.append(b)
		for c in C:
			if c <= h3 - overlap or c >= h3 + overlap: C2.append(c)
		A, B, C = A2, B2, C2
...
innerDir 	= os.path.join( outDir, 'bestWindows%d/' % windowSize)
...
for f in os.listdir( outDir ):
	...
	for w in windows:
		a_r = ' '.join( [ str(x) for x in np.arange( w[1][0], w[1][0] + windowSize )] )
		b_r = ' '.join( [ str(x) for x in np.arange( w[1][1], w[1][1] + windowSize )] )
		c_r = ' '.join( [ str(x) for x in np.arange( w[1][2], w[1][2] + windowSize )] )
