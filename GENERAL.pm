...
=head2
 ...
 Title   :  dot
 Usage   :  my $dp = GENERAL::cross(\@vec1, \@vec2);
...
=head2
 ...
 Title   :  mod
 Usage   :  my $dp = GENERAL::dot($n, $base);
...
#    my $mindex = 0;
    ...
    foreach my $dir (@list) {
      if (-d "$tmp_root/$dir") {
        # Was any file in this directory modified less than 1 day ago?
        ...
        if ($new_files eq "") {
          # if no, use it
          $LOCAL_DEF = "$LOCAL_DEF/$tmp_root/$dir";
...
#          if ($mindex < $1) { $mindex = $1; }
  ...
  GENERAL::csystem("touch $LOCAL_DEF/.taken"); # before lock is released, create a new file so that other processes know this directory is taken
...
sub destroyLocalSpace {
  my $username = GENERAL::GetUser();
  my $lfh = GENERAL::getFileLock("$LOCAL_DEF/../../../.energy\_$username.lock");
  GENERAL::csystem("rm -r $LOCAL_DEF");
  GENERAL::releaseFileLock($lfh);
  undef $LOCAL_DEF;
}
...
# function: Trim
# description: delete leading and trailing white space
# usage: &Trim(@ARGV);
sub Trim {
